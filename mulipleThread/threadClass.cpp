Mat MultipleThread::CustomThread(Mat src,int threadIndex){
    ...
    for (int i =0;i<threadIndex;++i){
        MatThreadProcess[i]=thread(&colorFilter::ThreadUseColorRule,&cfProcess,src(initRect),std::ref(dstThreadMat.at(i)));
    ...
    for(int j = 0;j<threadIndex-1;++j){
        if(j==threadIndex-2){
            vconcat(dstThreadMat.at(j), dstThreadMat.at(j+1),DstTemp);
        }
        vconcat(dstThreadMat.at(j), dstThreadMat.at(j+1),dstThreadMat.at(j+1));
    }
    return DstTemp;
}
